...
class TableExporter(http.Controller):
    ...
    @http.route('/matrix_view/matrix/check_xlsxwriter', type='json', auth='none')
    def check_xlsxwriter(self):
        return xlsxwriter is not None

    @http.route('/matrix_view/matrix/export_xlsx', type='http', auth="user")
    def export_xlsx(self, data, **kw):
        ...
        filename = osutil.clean_filename(_("Matrix %(title)s (%(model_name)s)", title=jdata['title'], model_name=jdata['model']))
